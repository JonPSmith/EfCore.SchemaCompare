...
- EfCore.SchemaCompare version 5 works with EF Core 5, 
- EfCore.SchemaCompare version 6 works with EF Core 6, etc.

_If you are looking for the version of EfCore.SchemaCompare for EF Core version 2 or 3, you can find it in the [EfCore.TestSupport version 3.2.0](https://www.nuget.org/packages/EfCore.TestSupport/3.2.0) (NOTE: This older version has most, but not all of the features that are supported by the latest version)._

The EfCore.SchemaCompare library (shortened to EfSchemaCompare in the documentation) is available on [NuGet as EfCore.SchemaCompare](https://www.nuget.org/packages/EfCore.SchemaCompare/).
Is an open-source library under the MIT licence.
See [ReleaseNotes](https://github.com/JonPSmith/EfCore.SchemaCompare/blob/master/ReleaseNotes.md) for details of changes and information on versions from EF Core 5 onwards.
...
- **Table/View:** Checks if a table or view that an entity class is mapped to exists. This checks both the table/view name and the schema name.
- **Property/Column:**  Checks the existence of properties, their column types in the database (including size and precision), nullability, computed column (including persistence), column's default value, when updated (things like columns that autogenerated by the DB on insert).
...
### Stage 2 - checks on the database

- Unused tables or views
- Unused columns
- Unused Indexes

### It understands the following EF Core features while doing the checks:
...
- The EF Core's scaffolder doesn't read in any index on the foreign key (it assumes EF Core will do that by default). That means it's unable to check for an index on a foreign key.
- It cannot correctly check Table-per-Type or Table-per-Class classes because EF Core doesn't currently hold that data. This is tracked by [Ef Core #19811](https://github.com/dotnet/efcore/issues/19811).
- It cannot compare database tables/columns using InvariantCultureIgnoreCase. That is an EF Core 5+ limitation.
- At the moment this library only supports SQL Server, SQLite and PostgreSQL.

The following are the things it doesn't bother checking:
...
EfSchemaCompare uses two EF Core features to get EF Core's internal schema and the database's schema.
Those are the following:

1. EF Core's `Model` property in your application's DbContext. This provides the internal version of the database schema that EF Core builds by looking at the entity classes and any model configuration attributes/methods.
2. EF Core's [Reverse Engineering](https://docs.microsoft.com/en-us/ef/core/managing-schemas/scaffolding) service, which allows the library to access an actual database schema.

The fun part is comparing these two sources, especially with all the different types of configurations that EF Core can handle.
The diagram shown below shows what happens when you use EfSchemaCompare to check a test database that you updated with some SQL migration scripts against the current EF Core's `Model`.
...
I recommend running the EfSchemaCompare code in a unit test, but how you do it is up to you.

Here is a example of how I would use the EfSchemaCompare feature:
...
public void CompareBookThenOrderAgainstBookOrderDatabaseViaAppSettings()
{
    ...
    using (var context2 = new OrderContext(options2))
    {
        ...
        //ATTEMPT
        //It starts with the connection string/name and then you can have as many contexts as you like.
...
Here is another error coming from stage 2 where it has detected an error on the database side.

```text
EXTRA IN DATABASE: Table 'MyEntites', column name. Found = MyEntityId
```

This says that there is a column named `MyEntityId` in the table `MyEntites` that hasn't got a property in an entity class mapped to it.

*NOTE: When an error contains the word `Table`, it refers to either a SQL Table or a View.*

## How to suppress certain error messages

In a few cases you will get false errors (see limitations) or just errors that you don't find relevant.
In these cases you might want to suppress those errors, which can be done by using the `CompareEfSqlConfig` class.

### Suppress errors via `IgnoreTheseErrors`

The first approach is the easier of the two.
Here, you capture the error strings you want to ignore into the configuration's `IgnoreTheseErrors`, with each error separated by a new line.
...
The other approach is useful when you want to suppress a general set of errors, but it is somewhat more complicated.
Here is an example where it suppresses any errors about the default value of a column.
...
There is one other configuration property called `TablesToIgnoreCommaDelimited`, which allows you to control which table/views in the database are to be considered. 

By default (i.e. when `TablesToIgnoreCommaDelimited` is null), `CompareEfSql` will only look at the tables/views in the database that your EF Core entity classes are mapped to.
This provides a good starting point. The other options are:
...
If there are tables/views in your database that EF Core doesn't access, then you need to inform `CompareEfSql` about them, lest it output an error warning you about them.
You do this by providing a comma delimited list of table names that are to be ignored, with an optional schema name if needed.
Here are two examples of a table name
  - `MyTable` - this has no schema, so the default schema of the database will be used;
  - `dbo.MyTable` - this refers to the database in the `dbo` schema.

*NOTE: The table name comparison is case insensitive.*  

Here is an example of how to configure the comparer to ignore the tables `Orders` and `LineItem`:
```c#
var config = new CompareEfSqlConfig
{
